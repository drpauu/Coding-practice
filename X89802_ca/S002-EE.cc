...
private:
  ...
  nat busca_node(int k) const;
};

nat dicc::busca_node(int k) const {
  nat i = h(k) % _M;
  ...
  if (p == NULL) {
    // La clau no es troba al diccionari
    return _M;
  } else {
    // La clau es troba al diccionari
    return i;
  }
}

void dicc::interseccio(const dicc &d2, dicc &res) const {
  // Recorrem totes les claus del diccionari actual
  for (nat i = 0; i < _M; ++i) {
    node_hash *p = _taula[i];
    while (p != NULL) {
      // Comprovem si la clau es troba a d2
      if (d2.busca_node(p->_k) != _M) {
        // Si es troba, la inserim a res
        res.insereix(p->_k);
      }
      p = p->_seg;
    }
