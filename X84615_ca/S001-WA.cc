...
vector<nat> dicc::freq_longituds() const {
    vector<nat> freq(longitud_maxima_clau(_arrel) + 1, 0); // Inicialitzem el vector amb la mida de la clau més llarga més un
    recorre_trie(_arrel, 0, freq);
    return freq;
}

void dicc::recorre_trie(node* t, nat longitud, vector<nat>& freq) {
    if (t == NULL) return;
    if (t->_c == '#') { // Si es troba el símbol de final de clau, incrementem la freqüencia de la longitud
        freq[longitud]++;
    }
