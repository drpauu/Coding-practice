...
void dicc::redispersio() {
  ...
  for (nat i = 0; i < _M; ++i) {
    if (_taula[i]._est == ocupat) {
      ...
      nat p = busca_node(_taula[i]._k);
      // ...i la marca com ocupada amb la clau.
      temp[p]._est = ocupat;
      temp[p]._k = _taula[i]._k;
    }
  }
  // Actualitzem la mida de la taula i assignem la taula temporal a la taula
  // original.
  _M = 2 * _M + 1;
  delete[] _taula;
  _taula = temp;
